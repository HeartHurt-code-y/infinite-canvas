...
- 被 `@` 引用的已连线素材实例按芯片首次出现顺序占据输入位置，同一实例只占一个位置；未被 `@` 引用的已连线实例按稳定连线顺序追加。
...
- 当前粘贴一律降为纯文本并重新按当前连线自动识别；不能从剪贴板 HTML 冒充精确引用身份。
...
2. 按提示内容中芯片首次出现顺序收集被引用实例；同一 `canvasNodeKey` 出现多次仍只进入媒体输入一次。随后把未被 `@` 引用的连线实例按 `edgeOrder` 追加。
...
`@` 只为已经连入当前生成节点的具体素材实例增加提示语义和排序优先级，不创建脱离连线的额外媒体输入。为避免引用错乱和隐藏重排，MVP 使用固定规则：

1. 分别建立 `images[]`、`videos[]` 和 `audios[]` 三个有效输入序列；三种媒体都从位置 `1` 独立计数。
2. 每种媒体序列内部，先按该类型目标实例的 `@` 芯片首次出现顺序排列，再按 `edgeOrder` 追加该类型未被引用的连线实例。
3. 不同媒体类型的芯片可以在提示文字中交错出现，但不会共用一个全局序号。例如依次出现“图片 A、视频 A、图片 B、音频 A”时，位置分别是 `图片1、视频1、图片2、音频1`。
...
6. `@` 图片在视频生成中默认映射为 `reference_image`，视频映射为 `reference_video`，音频映射为 `reference_audio`。
...
| 输入图片 | 图片     | 1～多张 | 从素材实例连入；被 `@` 引用的实例优先，其余实例按稳定连线顺序追加 |
...
- `@` 不产生额外图片，只精确指向已连线实例并决定其优先顺序。
- 每张图片在节点内显示缩略图和从 `1` 开始的顺序编号；被引用实例按芯片首次出现顺序在前，未引用实例按稳定连线顺序追加。
...
1. 按“被 `@` 引用的已连线实例在前、未引用连线实例在后”的规则形成图片输入列表；同一 `canvasNodeKey` 只保留一个 `图片 N` 位置。
...
| `doubao-seedance-2-5-260628`      | 暂按 `480p`、`720p`           | `4`～`30` 秒 | 最多 50 个多模态素材、纯音频参考、多语言、输出格式、任务类型引导 |

Seedance 2.5 的文档顶部模型表写有 `1080p`，但后续分辨率章节和专属说明都写明只支持 `480p`、`720p`。前端把 `480p`、`720p` 作为推荐选项，不自行声称 `1080p` 一定不可用；最终由后端或供应商返回真实结果。
...
| 联网搜索 `tools.web_search`         | Seedance 2.0 标准版、快速版 | 只在没有媒体输入的文生视频场景启用                         |
...
例如提示内容中的引用依次是 `@图片A、@视频A、@图片B、@音频A`，渲染标记必须依次为 `[图片1：图片A]、[视频1：视频A]、[图片2：图片B]、[音频1：音频A]`。文本片段继续按提示内容排列；协议若需要一个混合媒体列表，Adapter 使用固定展开规则构造，但位置语义只取自图片、视频和音频三套独立序列。特殊角色输入也遵守“被 `@` 引用的实例优先、其余实例按 `edgeOrder` 追加”的规则，并取得明确的 `图片 N`、`视频 N` 或 `音频 N` 位置；角色本身不能重新排序。
...
MVP 单素材导入优先使用 `POST /v1/assets/async`：它立即返回真实素材 ID，之后通过 `POST /v1/assets/get` 轮询 `Pending → Active/Failed/Deleted`。

旧的 `POST /v1/assets` 同步模式可能阻塞几十秒，不作为桌面交互的默认方式。旧异步模式和批量模式保留为后续能力；批量上传不是第一版必需功能。

素材状态在不同端点中使用 `Pending` 或 `Processing` 表示处理中，画布统一映射为 `processing`。
...
5. 应用取得同一对象的短期预签名 GET URL，交给 `POST /v1/assets/async`。
...
- 图片素材卡片可以连入提示词节点做视觉理解：执行生成、优化或审计时，后端按连线顺序取回素材字节（云端素材经素材库接口、本地素材经对象存储重签地址），以 Base64 图片内容块随用户消息发给所选文本模型；仅支持 png/jpeg/webp/gif，视频和音频素材连入提示词节点无效。
- 提示词节点输出框支持直接编辑；右下角“审计”会把完整技能和已保存的多轮上下文作为系统提示词，并固定发送“严格审查这个Phase 是否已经是最优版本”，返回 Diff 后由用户确认应用、保留或继续审计。
...
8. 画布按 `@` 芯片首次出现顺序和未引用连线的 `edgeOrder` 冻结最终素材列表，应用按该顺序读取图片字节并作为多个 `image[]` part 提交；前端不自设数量、格式或大小上限。
...
41. 把多张图片连到图片生成节点后提交，验证被 `@` 引用的实例按芯片首次出现顺序进入 multipart `image[]`，未引用实例按稳定连线顺序追加；任务详情能对应查看芯片、目标实例、`图片 N` 位置、内容哈希和实际文件 part。
...
7. 澄清 Seedance 2.5 是否支持 `1080p`；当前文档前后矛盾。
8. 澄清 `duration=-1` 是否支持 2.0 系列、2.5 编辑参考视频究竟允许 `2`～`15` 秒还是 `4`～`30` 秒，以及延长任务是否强制 `duration=-1`。
9. 澄清素材 `/v1/assets/async` 完成后返回的 `url` 是永久 OSS 地址还是约 12 小时签名地址；画布无论如何都以 `asset://` 作为稳定身份。
