...
视频节点的「白模导演台」可在实时 3D 视口里摆位、取景、运镜，也可一键从参考视频捕捉动作；预览与成片共用同一套逐帧烘焙数据。点「使用白模视频」后，继续配置专业级白模控制中的角色、道具和场景对应关系。

内置人形带关节骨架：自动走位、姿势预设，或把视频动捕片段贴到角色上。复杂场景仍可通过「在 Blender 中精修工程」完成：保存 `.blend` 后，选择「渲染 Blender 工程」回到应用渲染。导入模式沿用活动场景中的对象、动画和摄影机，按导演台指定的片长、帧率与画幅导出。
...
`input.json` 为 `{sourceBlendPath: null | string, plan: WhiteModelScenePlan, bake: WhiteModelBake | null}`，类型定义见 `src/lib/whiteModelStudio.ts`。应用端烘焙每一帧的机位与角色（含人形关节），脚本只负责把烘焙样本写成关键帧并渲染。脚本重新验证版本、数值、坐标、形状、颜色、路径时间和烘焙长度。输入始终作为数据读取。新场景的模型原点位于底部；坐标使用 Blender Z 轴向上，朝向为绕 Z 轴的角度。
