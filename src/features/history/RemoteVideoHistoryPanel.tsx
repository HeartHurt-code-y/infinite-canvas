...
type SettingsClient = Pick<ProviderSettingsClient, "listProviderConnections" | "listProviderTokenGroups">;
...
export function RemoteVideoHistoryPanel({
  ...
  const [groups, setGroups] = useState<{ providerId: string; items: readonly ProviderTokenGroup[] } | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    void settingsClient.listProviderConnections().then((items) => {
      if (cancelled) return;
      const enabled = items.filter((item) => item.enabled);
      setProviders(enabled);
      setProviderId((current) => enabled.some((item) => item.id === current) ? current : enabled[0]?.id ?? "");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    void settingsClient.listProviderTokenGroups(providerId).then((items) => {
      if (cancelled) return;
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      void loadPage({
        ...
        startTimestamp: parsed.createdFrom == null ? (parsed.createdTo == null ? 0 : null) : Math.floor(parsed.createdFrom / 1_000),
  ...
  const resumePolling = async (taskId: string) => {
    ...
    try {
      ...
      if (mountedRef.current && requestId === requestRef.current) {
        setNotice("已请求继续查询，本地任务会在后台更新；生成完成后自动保存视频。可查看本地记录或刷新列表。");
      }
    } catch (reason: unknown) {
  ...
  const useVideoUrl = async (url: string, copy: boolean) => {
    ...
    try {
      ...
      if (mountedRef.current && requestId === requestRef.current) setNotice(copy ? "视频地址已复制。" : "已打开视频地址。");
    } catch (reason: unknown) {
      if (mountedRef.current && requestId === requestRef.current) setError(errorText(reason));
    }
  };
  ...
  return (
    <div className="remote-video-history" role="tabpanel" id="remoteVideo-history-panel" aria-labelledby="remoteVideo-history-tab">
      <form className="remote-video-history__filters" onSubmit={submit}>
        <label>供应商
          <select value={providerId} onChange={(event) => {
            invalidateQuery(); setProviderId(event.target.value); setTokenGroup(""); setGroups(null); setSettingsError(null);
          }}>
            {!providers.length ? <option value="">{providersLoaded ? "暂无启用的供应商" : "正在加载供应商…"}</option> : null}
            {providers.map((provider) => <option key={provider.id} value={provider.id}>{provider.displayName}</option>)}
          </select>
        </label>
        <label>令牌分组
          <select value={tokenGroup} disabled={!canQuery} onChange={(event) => { invalidateQuery(); setTokenGroup(event.target.value); }}>
            <option value="">默认令牌</option>
            {availableGroups.map((group) => <option key={group.id} value={group.groupName}>{group.groupName}</option>)}
          </select>
        </label>
        <label>提交开始时间
          <input type="datetime-local" step="1" value={range.from} onChange={(event) => { invalidateQuery(); setRange({ ...range, from: event.target.value }); }} />
        </label>
        <label>提交结束时间
          <input type="datetime-local" step="1" value={range.to} onChange={(event) => { invalidateQuery(); setRange({ ...range, to: event.target.value }); }} />
        </label>
        <label>远程状态
          <select value={status} onChange={(event) => { invalidateQuery(); setStatus(event.target.value as RemoteVideoTaskStatus | ""); }}>
            <option value="">全部状态</option>
            {Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}
          </select>
        </label>
        <div className="remote-video-history__actions">
          <button type="submit" className="remote-video-history__primary" disabled={!canQuery || loading}>{loading ? "查询中…" : "查询远程任务"}</button>
          <button type="button" onClick={() => { invalidateQuery(); setRange(todayRange()); setStatus(""); }}>今天</button>
          <button type="button" onClick={() => { invalidateQuery(); setRange({ from: "", to: "" }); }}>全部时间</button>
        </div>
      </form>
      <p className="remote-video-history__hint">按提交时间查询，使用本机时区。应用关闭期间的任务仍可在这里查询；选择提交时使用的供应商和令牌。仅查询视频任务。</p>
      {settingsError ? <div className="remote-video-history__error" role="alert">{settingsError} <button type="button" onClick={() => { setSettingsError(null); setGroups(null); setSettingsRevision((value) => value + 1); }}>重试加载配置</button></div> : null}
      {providersLoaded && !providers.length && !settingsError ? <p className="remote-video-history__hint">请先在供应商设置中配置并启用连接。</p> : null}
      {error ? <p ref={errorRef} tabIndex={-1} className="remote-video-history__error" role="alert">{error}</p> : null}
      {notice ? <p className="remote-video-history__notice" role="status">{notice}</p> : null}
      <div className="remote-video-history__results" aria-busy={loading}>
        {!page && !loading && !error ? <p className="history-list__empty">选择查询条件后，点击“查询远程任务”。</p> : null}
        {loading ? <p role="status">正在查询远程任务…</p> : null}
        {page ? <>
          <div className="remote-video-history__pagination">
            <span role="status">共 {page.total} 条 · 第 {page.page} / {totalPages} 页</span>
            <div className="remote-video-history__actions">
              <button type="button" disabled={loading || !appliedQuery} onClick={() => appliedQuery && void loadPage(appliedQuery)}><ArrowClockwise size={14} aria-hidden="true" /> 刷新</button>
              <button type="button" disabled={loading || page.page <= 1 || !appliedQuery} onClick={() => appliedQuery && void loadPage({ ...appliedQuery, page: page.page - 1 })}>上一页</button>
              <button type="button" disabled={loading || page.page >= totalPages || !appliedQuery} onClick={() => appliedQuery && void loadPage({ ...appliedQuery, page: page.page + 1 })}>下一页</button>
            </div>
          </div>
          {!page.items.length ? <p className="history-list__empty">该时间范围和状态下没有视频任务。</p> : null}
          <ul className="remote-video-history__list">
            {page.items.map((task, index) => <li key={`${task.taskId}:${task.submitTime}:${index}`} className="remote-video-history__item">
              <div className="remote-video-history__task-title"><strong>{task.taskId || "尚未取得远程任务 ID"}</strong><span>{STATUS_LABELS[task.status as RemoteVideoTaskStatus] ?? task.status}{task.progress ? ` · ${task.progress}` : ""}</span></div>
              <dl>
                <div><dt>提交</dt><dd>{dateTime(task.submitTime)}</dd></div>
                <div><dt>开始</dt><dd>{dateTime(task.startTime)}</dd></div>
                <div><dt>完成</dt><dd>{dateTime(task.finishTime)}</dd></div>
                <div><dt>Token</dt><dd>输入 {task.promptTokens} · 输出 {task.completionTokens}</dd></div>
              </dl>
