...
export function WorkflowReferenceMaterials({
  ...
  const atCapacity =
    allMaterials.length >= MAX_WORKFLOW_MATERIALS || totalBytes >= MAX_WORKFLOW_MATERIAL_BYTES;
  const overLimit =
    allMaterials.length > MAX_WORKFLOW_MATERIALS || totalBytes > MAX_WORKFLOW_MATERIAL_BYTES;
  const generalPaths = new Set(materials.map(workflowMaterialPathKey));
  const canReuseSpecialized = allMaterials.some(
    (material) => !generalPaths.has(workflowMaterialPathKey(material)),
  );
  ...
  return (
    ...
    >
      ...
      <p className="canvas-workflow-references__quota" aria-live="polite">
        已添加 {materials.length} 项 · 全部参考资料 {allMaterials.length} / {MAX_WORKFLOW_MATERIALS}{" "}
        项 · {formatBytes(totalBytes) ?? "0 B"} / 14 MB
      </p>
