...
export interface WhiteModelViewportHandle {
...
export interface WhiteModelViewportProps extends WhiteModelViewportCallbacks {
  ...
  readonly environmentSrc?: string | null;
  readonly showDummyLabels?: boolean;
}
 ...
 */
export const WhiteModelViewport = forwardRef<WhiteModelViewportHandle, WhiteModelViewportProps>(
  function WhiteModelViewport(
    {
      plan,
      clock,
      view,
    ...
    useImperativeHandle(ref, () => ({
      captureStill: () => controllerRef.current?.captureStill() ?? null,
    }));
      ...
      void import("./whiteModelViewportController")
        .then((module) => {
          ...
          controller = new module.WhiteModelViewportController(canvas, clock, {
            ...
            onActorMove: (id, position, time) =>
              callbacksRef.current.onActorMove(id, position, time),
    ...
    useEffect(() => {
      controllerRef.current?.setEnvironmentSrc(environmentSrc);
    }, [environmentSrc, ready]);
    useEffect(() => {
      controllerRef.current?.setShowDummyLabels(showDummyLabels);
    }, [showDummyLabels, ready]);
    useEffect(() => {
