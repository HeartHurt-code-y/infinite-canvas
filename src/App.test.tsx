...
describe("App workspace", () => {
  it("renders an empty canvas and the generation templates", () => {
    ...
    expect(repository.querySelectorAll(".repository-card")).toHaveLength(8);
    ...
    expect(templates[4]).toHaveAccessibleName("拖拽创建爆款视频复刻节点");
    expect(templates[5]).toHaveAccessibleName("拖拽创建提示词生成与优化节点");
    expect(templates[6]).toHaveAccessibleName("拖拽创建剧本创作与优化节点");
    expect(templates[7]).toHaveAccessibleName("拖拽创建剧本转工业级分镜脚本节点");
